...
import com.jetpack.reader.app.components.EmailInput
import com.jetpack.reader.app.components.PasswordInput
...
fun ReaderLoginScreen(navController: NavController,viewModel:LoginScreenViewModel= androidx.lifecycle.viewmodel.compose.viewModel()) {
    ...
    Surface(modifier = androidx.compose.ui.Modifier.fillMaxSize()) {
        ...
        verticalArrangement = Arrangement.Center) {
            ReaderLogo(logoSize = 120, isSplashScreen = false)
...
val passwordFocusRequest=FocusRequester.Default
    ...
    Column(modifier, horizontalAlignment = Alignment.CenterHorizontally) {
        if (isCreateAccount) Text(text = stringResource(id = R.string.create_act),modifier=Modifier.padding(4.dp)) else Text("")
